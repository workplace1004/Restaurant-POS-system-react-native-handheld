...
const ticketText = 'text-[14px] leading-tight text-pos-bg';
const ticketTextSemi = 'text-[14px] leading-tight text-pos-bg font-semibold';
const ticketNote = 'text-[14px] leading-tight text-pos-bg opacity-90';
...
export function OrderPanel({ order, orders, onRemoveItem, onUpdateItemQuantity, onStatusChange, onCreateOrder, onRemoveAllOrders, tables, showSubtotalView = false, subtotalBreaks = [], onPaymentCompleted, selectedTable = null, currentUser = null, currentTime = '', onOpenTables, quantityInput = '', setQuantityInput, showInWaitingButton = false, showInPlanningButton = true, onOpenInPlanning, onOpenInWaiting, onSaveInWaitingAndReset, focusedOrderId = null, focusedOrderInitialItemCount = 0 }) {
  ...
  return (
    <View className="flex-1 min-h-0 w-full flex flex-col px-2 py-1 bg-pos-bg">
      ...
      <View className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg bg-white">
        {customerDisplayName ? (
          <View className="px-2 py-1 text-center border-b border-pos-border">
            <Text className={`${ticketText} font-medium truncate block text-center w-full`}>{customerDisplayName}</Text>
          </View>
        ) : null}
        {showSubtotalView ? (
              ...
              <View key={`saved-order-${savedOrder.id}`}>
                {(savedOrder.items || []).map((item) => (
                  <View
                    ...
                    className="mb-1 rounded px-2 py-2"
                  >
                  ...
                  {(() => {
                    ...
                    return (
                      <View className="flex items-center justify-around py-0.5">
                        <Text className={`${ticketTextSemi} opacity-90 text-center`}>{`${savedCashierName} - ${savedTime}`}</Text>
                      </View>
                    );
                  })()}
                  ...
                  return (
                    <React.Fragment key={`batch-${batchIdx}`}>
                      {batchItems.map((item) => (
                        <Pressable
                          key={item.id}
                          className={`mb-1 rounded-none px-2 py-2 active:bg-green-500 ${selectedItemIds.includes(item.id) ? 'bg-gray-300' : ''}`}
                      ...
                      <View className="px-1.5 pt-0.5">
                        <View className="flex items-center justify-around py-0.5">
                          <Text className={`${ticketTextSemi} opacity-90 text-center`}>{`${metaUserName} - ${metaTime}`}</Text>
                        </View>
                        <View className="w-full h-px bg-pos-bg/40" />
                      </View>
                    </React.Fragment>
      ...
      <View className="w-full min-w-0 flex-row items-center gap-1 border-t py-1 border-black/10 px-1">
          <Pressable
            disabled={!hasSelection}
            className={`min-w-0 flex-1 h-10 items-center justify-center rounded border-none ${
      ...
      {hasSelectedTable ? (
        showSettlementActions ? (
          ...
          <View className="flex py-2">
            ...
            >
              <Text className="text-pos-text text-center text-[12px]">{t('addToTable')}</Text>
            </Pressable>
          </View>
        )
      ) : (
        <View className="flex gap-1 py-1">
          ...
          <View className="w-full min-w-0 flex-row flex-nowrap items-center gap-2">
          <Pressable
        ...
        {KEYPAD.map((row, ri) => (
          <View key={ri} className="flex flex-row gap-1">
            {row.map((key) => (
              <Pressable
                key={key}
                className="min-w-0 flex-1 rounded-md border-none bg-pos-panel py-1.5 text-[14px] text-pos-text active:bg-green-500"
                onPress={() => handleKeypad(key)}
              >
                <Text className="text-pos-text text-center text-[14px]">{key}</Text>
              </Pressable>
            ))}
          </View>
        ))}
